...
class DrrnOptAgent(DrrnAgent):
    def __init__(
        ...
        self.llm_model = OPTForSequenceClassification.from_pretrained(
            "facebook/opt-350m", problem_type="multi_label_classification"
        )
    ...
    def act(self, states, poss_acts, poss_act_strs, sample=True):
        ...
        llm_input_tokens = self.llm_model(states, return_tensors="pt")
        logits = self.llm_model(**llm_input_tokens).logits
